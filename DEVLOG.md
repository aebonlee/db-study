...
## 2026-03-14 (Day 1-13) — 실력테스트 학습 가이드 기능 추가

### 변경 내용

- 초급/중급테스트 모든 문제에 **학습 가이드(guide)** 데이터 추가
- 시험 중 사이드바에 "📖 가이드 열기/닫기" 토글 버튼 + 가이드 패널 구현
- 문제별로 정답을 맞추기 위한 핵심 개념·키워드·공식을 표시
- 가이드 패널 전용 CSS 추가 (`.exam-guide-panel`, `.exam-guide-toggle`, `.exam-guide-content`)
- 시험 안내에 "학습 가이드 참고 가능" 문구 추가 (초급/중급 모두)

### 수정 파일

| 파일 | 변경 내용 |
|------|-----------|
| `DbExamBasic.jsx` | 26문항 전체에 `guide` 필드 추가 + 가이드 패널 UI + `showGuide` 상태 |
| `DbExamInter.jsx` | 50문항 전체에 `guide` 필드 추가 + 가이드 패널 UI + `showGuide` 상태 |
| `site.css` | 가이드 패널 CSS 5개 클래스 추가 |
