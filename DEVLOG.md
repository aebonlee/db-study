...
- **영역 구분 강화**: 3컬럼(문항/문제/가이드) 각 영역에 2px border + box-shadow 적용, 사이드바 배경색 분리
- **보기 번호 ①②③④**: 초급/중급 모두 객관식 보기를 원형 번호로 통일, 선택 시 색상 반전
- **문제 번호 강화**: "문제 N" 표시를 파란색 배지(badge) 스타일로 변경

### 수정 파일

| 파일 | 변경 내용 |
|------|-----------|
| `DbExamBasic.jsx` | "A." 접두어 제거 → ①②③④ 번호 표시 |
| `DbExamInter.jsx` | ①②③④ 번호 추가 |
| `site.css` | `.exam-sidebar` border/배경, `.exam-q-num` 배지, `.exam-option-num` 원형 번호, border 강화 |

---

## 2026-03-14 (Day 1-13) — 실력테스트 학습 가이드 + 3컬럼 레이아웃

### 변경 내용

- 시험 화면을 **2:5:3 비율 3컬럼 레이아웃**으로 변경
  - 좌측(2): 문항 네비게이션 + 진행 현황
  - 중앙(5): 문제 영역 (질문, 선택지/입력, 이전/다음)
  - 우측(3): 학습 가이드 (항상 표시)
- 초급/중급테스트 모든 문제에 **학습 가이드(guide)** 데이터 추가
- 문제별 핵심 개념·키워드·공식을 우측 패널에 상시 표시
- 모바일에서는 1컬럼으로 자동 전환
...
| 파일 | 변경 내용 |
|------|-----------|
| `DbExamBasic.jsx` | 26문항 `guide` 추가 + 3컬럼 가이드 영역 |
| `DbExamInter.jsx` | 50문항 `guide` 추가 + 3컬럼 가이드 영역 |
| `site.css` | 3컬럼 그리드(`2fr 5fr 3fr`) + 가이드 패널 CSS |

---

## 2026-03-14 (Day 1-12) — DB 실력테스트 초급/중급 분리 + 디자인 통일

### 변경 내용

#### 1. 기존 시험 → 초급테스트로 변경
- 기존 `/exam` 단일 메뉴를 드롭다운으로 변경
- 기존 26문항 시험을 `/exam/basic` (초급테스트)로 분리

#### 2. 중급테스트 신규 추가
- `db_final-main` 프로젝트 기반 50문항 중급테스트 (`/exam/intermediate`)
- 객관식 30문항(60점) + 단답식 20문항(40점) = 100점, 60분
- 키워드 기반 단답식 채점, 문제별 해설 포함

#### 3. 디자인 통일 (중급 → 초급 반영)
- **사이드바 레이아웃**: 좌측 네비게이션 + 점수 트래커, 우측 문제 영역
- **네비게이션**: 유형별 그룹 분리, 응답 상태 색상 표시
- **제출 확인 모달**: 답변/미답변 현황 표시 후 제출
- **결과 탭**: 유형별 탭 전환 (A형/B형/C형 또는 객관식/단답식)
- **해설 표시**: 문제별 상세 결과에 해설(explanation) 추가
- **목차 페이지**: `/exam`에서 초급/중급 카드형 선택 화면

#### 4. 메뉴 구조 변경
```
DB 실력테스트 ▼
├ 초급테스트  (/exam/basic)
└ 중급테스트  (/exam/intermediate)
```
...
| 파일 | 변경 내용 |
|------|-----------|
| `DbExam.jsx` | 시험 → 목차 페이지(초급/중급 카드 선택) |
| `DbExamBasic.jsx` | **신규** — 초급테스트 26문항 (사이드바 레이아웃 + 해설) |
| `DbExamInter.jsx` | **신규** — 중급테스트 50문항 (사이드바 레이아웃 + 실시간 점수 + 해설) |
| `site.js` | exam 메뉴를 드롭다운으로 변경 (examBasic, examInter) |
| `translations.js` | 초급테스트/중급테스트 번역 키 추가 |
| `PublicLayout.jsx` | DbExamBasic, DbExamInter lazy import + 라우트 추가 |
| `site.css` | 시험 CSS 전면 재작성 (Landing + 사이드바 + 모달 + 탭 + 해설) |

---

## 2026-03-14 (Day 1-11) — DB 실력테스트 메뉴 추가

### 추가 배경
D:\database_test (db_final) 프로젝트의 데이터베이스 시험 시스템을 React 컴포넌트로 변환하여 "DB 실력테스트" 메뉴로 통합.

### 시험 구성

| 유형 | 문항 수 | 배점 | 문제 유형 |
|------|---------|------|-----------|
| A형 (기본 이론) | 10문항 | 20점 (각 2점) | 객관식 + 단답형 |
| B형 (서술형) | 8문항 | 40점 (각 5점) | 서술형 |
| C형 (SQL 실기) | 8문항 | 40점 (각 5점) | SQL 작성 |
| **합계** | **26문항** | **100점** | 제한시간 60분 |

### 주요 기능
- 3단계 진행: 응시자 정보 입력 → 시험 진행 → 결과 확인
- 60분 카운트다운 타이머 (자동 제출)
- 26문항 네비게이션 그리드 (응답 상태 표시)
- 키워드 기반 채점 (서술형/SQL): 70%↑ 만점, 40~69% 70점, 20~39% 40점
- 결과 화면: 총점/등급 + 유형별 점수 + 문항별 상세 리뷰

### 수정 파일

| 파일 | 변경 내용 |
|------|-----------|
| `DbExam.jsx` | **신규** — 26문항 DB 시험 시스템 (전체 데이터 + 3단계 UI + 채점 로직) |
| `site.js` | menuItems에 "DB 실력테스트" 메뉴 추가 (참고자료 앞) |
| `translations.js` | ko: 'DB 실력테스트', en: 'DB Test' 추가 |
| `PublicLayout.jsx` | DbExam lazy import + `/exam` 라우트 추가 |
| `site.css` | ~80줄 시험 UI CSS 추가 (intro/exam/result 3단계) |

### 메뉴 구조 변경
```
DB 개론 ▼ | DB 서비스 종류 ▼ | DB 웹 연동 ▼ | SQL 학습 ▼ | DB 튜닝 ▼ | Oracle 튜닝 ▼ | DB 실력테스트 | 참고자료 | 교육신청
```

---

## 2026-03-14 (Day 1-10) — 교육신청 박스형 디자인 + 푸터 바로가기 2열 배치

### 변경 내용

#### 1. 교육신청 페이지 박스형 디자인
- 기존 카드 그리드 → 가로형 박스 카드(tuning-card-box 재사용)로 변경
- 난이도 배지(입문/중급/고급/맞춤) + 아이콘 + 제목/설명 + 키워드 태그 구성
- 문의 연락처를 4열 카드 그리드로 재배치 (이메일/전화/카카오톡/운영시간)

#### 2. 푸터 바로가기 상단 메뉴 기반 2열 배치
- 기존: 별도 footerLinks 배열에서 5개 링크
- 변경: 상단 menuItems에서 자동 추출하여 2열 그리드 배치
- `footer-nav-links` 클래스 추가 (`grid-template-columns: 1fr 1fr`)

### 수정 파일

| 파일 | 변경 내용 |
|------|-----------|
| `Training.jsx` | 박스형 카드 디자인으로 전면 재작성 |
| `Footer.jsx` | 바로가기를 menuItems 기반 2열 그리드로 변경 |
| `site.css` | training CSS 전면 재작성 (박스형 + 문의 카드 그리드) |
| `footer.css` | footer-nav-links 2열 그리드 스타일 추가 |
