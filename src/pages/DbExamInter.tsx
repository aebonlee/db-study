...
interface QuestionDetail extends QuestionBase {
...
/* ─── 채점 ─── */
function scoreQuestion(q: QuestionBase, userAns: string | number | undefined): number {
  ...
  if (q.keywords) {
    return q.keywords.some((kw: string) => input === kw.toLowerCase() || input.includes(kw.toLowerCase())) ? q.pts : 0;
  }
  return input === String(q.ans).toLowerCase() ? q.pts : 0;
}

/* ─── 메인 컴포넌트 ─── */
const DbExamInter = (): ReactElement => {
  useAOS();
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { if (timerRef.current !== null) clearInterval(timerRef.current); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  const startExam = (): void => { if (studentName.trim()) setPhase('exam'); };

  const saveAnswer = useCallback((val: string | number) => {
    setAnswers(prev => ({ ...prev, [cur]: val }));
  }, [cur]);

  const getAnsweredCount = (): number => Object.keys(answers).filter(k => answers[Number(k)] !== '' && answers[Number(k)] !== undefined).length;
  ...
  const handleSubmit = (): void => {
    ...
    let total = 0;
    const cats: Record<string, CatScore> = {};
          ...
          <div className="exam-intro" data-aos="fade-up">
            ...
            <div className="exam-start-form">
              <input type="text" placeholder="이름을 입력하세요" value={studentName}
                ...
                onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => e.key === 'Enter' && startExam()}
        ...
        <section className="section"><div className="container">
            ...
            <div className="exam-question-area">
              ...
              {q.type === 'multiple' ? (
                ...
                <textarea className="exam-textarea" rows={3}
                  ...
                  value={answers[cur] || ''}
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => saveAnswer(e.target.value)} />
              )}
          ...
          {showConfirm && (
            <div className="exam-modal-overlay" onClick={() => setShowConfirm(false)}>
              <div className="exam-modal" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
                <h3>시험 제출 확인</h3>
  ...
  const correctCount = details.filter((d: QuestionDetail) => d.score === d.pts).length;
        ...
        <div className="exam-result-tabs" data-aos="fade-up" data-aos-delay="150">
          {CATS.map(c => (
            <button key={c} className={`exam-tab-btn ${activeTab === c ? 'exam-tab-btn--active' : ''}`}
              onClick={() => setActiveTab(c)}>{c} ({details.filter((d: QuestionDetail) => d.cat === c).length})</button>
          ))}
        </div>

        <div className="exam-detail-list">
          {details.filter((d: QuestionDetail) => d.cat === activeTab).map((d: QuestionDetail) => (
            <details key={d.id} className={`exam-detail-item ${d.score === d.pts ? 'exam-detail--ok' : 'exam-detail--wrong'}`}>
              ...
              <div className="exam-detail-body">
                ...
                <pre className="exam-detail-ans">{d.type === 'multiple' ? (d.userAns !== undefined ? d.opts![d.userAns as number] : '(미답변)') : (d.userAns || '(미답변)')}</pre>
                <p><strong>정답:</strong></p>
                <pre className="exam-detail-ans exam-detail-correct">{d.type === 'multiple' ? d.opts![d.ans as number] : d.ans}</pre>
