...
function scoreQuestion(q: QuestionBase, userAns: string | number | undefined): number {
  ...
  if (!userStr.trim()) return 0;
  if (q.type === 'multiple') return userAns === q.ans ? q.pts : 0;
  const ansStr = String(q.ans);
  const keywords = ansStr.replace(/[.,;:!?()]/g, ' ').split(/\s+/).filter((w: string) => w.length >= 2);
  const input = userStr.toLowerCase();
  const matched = keywords.filter((kw: string) => input.includes(kw.toLowerCase()));
...
/* ─── 메인 컴포넌트 ─── */
const DbExamBasic = (): ReactElement => {
  useAOS();
      ...
      <section className="section"><div className="container">
        <div className="exam-result-summary" data-aos="fade-up">
          ...
          <div className="exam-score-info">
            ...
            <span>정답률: {Math.round(details.filter((d: QuestionDetail) => d.score === d.pts).length / questions.length * 100)}%</span>
          </div>
        </div>
